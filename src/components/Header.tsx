...
export const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 ${currentTheme.header} border-b ${currentTheme.text} ${currentTheme.font} transition-all duration-500`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className={`transition-colors duration-200 hover:opacity-80 ${pathname === href ? `font-semibold ${currentTheme.title}` : `${currentTheme.textSecondary}`}`}
              >
                {label}
              </Link>
            ))}
                  ...
                  {['light', 'dark', 'colorful'].map((t) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm transition-colors duration-200 first:rounded-t-lg last:rounded-b-lg hover:bg-opacity-70 ${
                        theme === t
                          ? `font-semibold ${currentTheme.title}`
        ...
        {isMobileMenuOpen && (
          <div
            className={`md:hidden mt-2 pt-4 pb-4 space-y-2 ${currentTheme.card} rounded-lg transition-all mb-4`}
          >
            {navItems.map(({ href, label }) => (
              <Link
                key={href}
                ...
                className={`block py-2 px-4 rounded-lg ${pathname === href ? `font-semibold ${currentTheme.title}` : `${currentTheme.textSecondary}`}`}
              >
                {label}
              </Link>
            ))}
